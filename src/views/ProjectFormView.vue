...
async function handleDelete() {
  if (
    isEditMode.value &&
    confirm(`Apakah Anda yakin ingin menghapus proyek "${form.value.title}"?`)
  ) {
...
<template>
  <div>
    <div class="flex items-center mb-6">
      ...
      <div>
        <h1 class="text-3xl font-bold text-brand-dark-blue">
          {{ isEditMode ? 'Ubah Project' : 'Tambah Project' }}
        </h1>
        <p class="text-sm text-gray-500">
          Website / Our Projects / {{ isEditMode ? 'Edit' : 'Add' }}
        </p>
      </div>
    </div>
    ...
    <form @submit.prevent="handleSubmit" class="p-8 bg-white rounded-lg shadow-md space-y-8">
      ...
      <fieldset class="border-2 border-dashed p-4 rounded-md">
        <legend class="text-lg font-medium px-1 text-brand-dark-blue">
          Pengaturan Hero Section
        </legend>
        <p class="text-sm text-gray-500 mb-4">
          Unggah gambar di sini jika Anda ingin project ini tampil di Hero Section landing page.
          Kosongkan jika tidak.
        </p>
        <ImageUploader label="Gambar untuk Hero" v-model="form.heroImageUrl" />
      </fieldset>
      <!-- BAGIAN TOMBOL AKSI -->
      <div class="flex justify-between items-center pt-6 border-t">
        <div>
          <button
            ...
            @click="handleDelete"
            type="button"
            class="px-6 py-2 text-sm font-bold text-white bg-red-600 rounded-md hover:bg-red-700"
          >
            Hapus Project
          </button>
        </div>
